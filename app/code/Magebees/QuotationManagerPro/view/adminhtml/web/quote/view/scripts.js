...
define([
    ...
    AdminQuote.prototype = {
        ...
        setCustomerId: function (id) {
           ...
           jQuery(this.getAreaId('header')).callback = 'setCustomerAfter';
			  this.customerSelectorHide();
            if (this.storeId) {
                jQuery(this.getAreaId('data')).callback = 'dataLoaded';
            ...
            jQuery('back_quote_top_button').hide();
            jQuery('reset_quote_top_button').show();
        },
        ...
        setStoreId: function (id) {
			...
			//var area=['header', 'sidebar','data'];
			var area=['header','data'];
          ...
          var  params = this.prepareParams(params);
            params.json = true;
  			new Ajax.Request(url, {
                    parameters: params,                   
                    onSuccess: function (transport) {
                     var response = transport.responseText.evalJSON();
					jQuery("#quote-data").html(response.data);	
						this.storeSelectorHide();
                       this.dataShow();
                  /* var quote_view_url=this.loadBaseUrl.replace("quote_create/loadBlock", "quote/view/quote_id/"+response.header);
						window.location.href =quote_view_url;*/
        ...
        changeAddressField: function (event) {
            ...
            var name = matchRes[2];
            var data;
if(jQuery('#quote-shipping_address_'+name).attr('disabled'))
			{
				var field_id=field.id;				
				var ele_val=jQuery('#'+field_id).val();			
				jQuery('#quote-shipping_address_'+name).val(ele_val);
			}
            ...
            /*var customerAddressId = $('quote-' + type + '_address_customer_address_id').value;
            data['quote[' + type + '_address][customer_address_id]'] = customerAddressId;*/
            ...
            if (data['reset_shipping']) {
                ...
                if (name == 'country_id' || name == 'customer_address_id') {
                   // this.loadArea(['shipping_method', 'billing_method', 'totals', 'convert', 'items'], true, data);
                    this.loadArea(['shipping_method_form'], true, data);
                }
...
//var areasToLoad = ['billing_method', 'shipping_address', 'totals', 'convert', 'giftmessage'];
            var areasToLoad = ['shipping_address_form'];
        ...
        loadShippingRates: function () {
			var action = jQuery('#edit_form').attr('action');
			var quote_id =jQuery('#magebees_quote').val();			var self=this;
			action = action.replace("save", "createQuote");
						...
						showLoader:true,
					 	data: params,
							success: function(response){
								 self.isShippingMethodReseted = false;
            self.loadArea(['shipping_method', 'totals', 'convert'], true, {collect_shipping_rates: 1});
							}
					});
           
        },

        setShippingMethod: function (method,ele) {
			
			var applied_rate_incl_tax=jQuery(ele).closest("li").find(".rate_incl_tax").val();
			var applied_rate_excl_tax=jQuery(ele).closest("li").find(".rate_excl_tax").val();
			
            var data = {};
            data['quote[shipping_method]'] = method;
            data['quote[applied_rate_incl_tax]'] = applied_rate_incl_tax;
            data['quote[applied_rate_excl_tax]'] = applied_rate_excl_tax;
        ...
        applySubtotalProposal: function (amount, isPercentage) {
            this.loadArea(['sidebar', 'items', 'shipping_method', 'billing_method', 'totals', 'convert', 'giftmessage'], true, {
                'proposal[subtotal_proposal]': amount,
                'proposal[proposal_is_percentage]': isPercentage,
                reset_shipping: false
            });
        ...
        removeQuoteItem: function (id) {
            this.loadArea(['items', 'shipping_method', 'totals', 'convert', 'billing_method'], true,
        ...
        productGridAddSelected: function () {
            ...
            var area = ['search', 'items', 'totals', 'convert', 'giftmessage', 'billing_method','shipping_address','shipping_method'];
        ...
        selectCustomer: function (grid, event) {
			...
			var ele=$(element).select('.col-entity_id');
			   for (var i = 0; i < ele.length; i++) {
                      
                       this.setCustomerId(ele[i].innerText);
                    }
          /*  if (element.title) {
        ...
        itemsUpdate: function () {
            ...
            var info = jQuery('quote-items_grid').select('input', 'select', 'textarea');
        ...
        loadAreaResponseHandler: function (response) {
			...
			jQuery(".quote-totals").replaceWith((".quote-totals",response.totals));
jQuery("#quote-shipping-method-summary").html(response.shipping_method);
//jQuery(".quote-addresses").html(response.shipping_method_form);
			
			
	//	jQuery(".quote-addresses").replaceWith((".quote-addresses",response.shipping_address));
			jQuery(".quote_error").html(response.message);
            if (response.error) {
                alert({
            ...
            if (response.header) {
                ...
                jQuery('.page-title').html(response.header);
            }
        ...
        prepareParams: function (params) {
            ...
            //    var data = this.serializeData('quote-billing_method');
            //    if (data) {
        ...
        processCustomerGroupChange: function (groupIdHtmlId,
                                              message,
